package com.AmitSureshChandra.shippingservice.config;
...
public class AmqpConfig {
    ...
    @Bean
    public Queue initiateShipping() {
       return new Queue(appName);
    }

    @Bean
    Binding eventLogQueueBinding(Queue initiateShipping, TopicExchange exchange) {
        return BindingBuilder.bind(initiateShipping).to(exchange).with("payment-service.payment.order-paid-event");
    }
}
